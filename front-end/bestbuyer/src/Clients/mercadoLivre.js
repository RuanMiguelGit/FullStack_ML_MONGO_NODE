...
export const getDataFromMercadoLivre = async (source) => {
  ...
  const data = await getData(
    `https://api.mercadolibre.com/sites/${origin}/categories`
  ).then((res) =>
    res.map((item) => {
      return item;
    })
...
export const getProductsFromMercadoLivre = async (
  productName,
  mercadoLivre,
  source
) => {
  ...
  productName === ''
    ? (ProductId = '')
    : (ProductId = await mercadoLivre.find((item) => item.name === productName)
        .id);
  const data = await getData(
...
export const FormatMercadoLivreProducts = async (
  products,
  category,
  source
) => {
  ...
  let permalink = products.map((item) => item.permalink);
